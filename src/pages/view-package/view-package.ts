...
export class ViewPackagePage {
  ...
  constructor(
    ...
    (this.first = true), this.enableTimes(this.slots, moment());
  ...
  choosePkg() {
    ...
    if (!day)
      day = this.calander_day && {
        calander: true,
        label: "",
        value: moment(
          `${this.calander_day["year"]}-${this.calander_day["month"]}-${this.calander_day["day"]}`,
          "YYYY-MM-DD"
        )
      };
    ...
    this.navCtrl.push(SelectPackagePage, {
      sName: cc,
      dSlot: day.value ? day.value.format("YYYY-MM-DD 00:00:00") : "nothing",
      tSlot: time.value
    });
  }
